...
class addcourse(FlaskForm):
    ...
    dicop1 = DecimalField(
        "Direct-Indirect Attainment Ratio ", validators=[DataRequired(), NumberRange(min=0,max=100)]
    )
    dicop2 = DecimalField(
        "Direct-Indirect Attainment Ratio ", validators=[DataRequired(), NumberRange(min=0,max=100)]
    )
    scop1 = DecimalField(
        "Direct Attainment Ratio ", validators=[DataRequired(), NumberRange(min=0,max=100)]
    )
    scop2 = DecimalField(
        "Direct Attainment Ratio ", validators=[DataRequired(), NumberRange(min=0,max=100)]
    )
    ciet = DecimalField(
        "Internal-Test Percentage", validators=[DataRequired(), NumberRange(min=0,max=100)]
    )
    cieq = DecimalField(
        "Internal-Quiz Percentage", validators=[DataRequired(), NumberRange(min=0,max=100)]
    )
    assign = DecimalField(
        "Internal-Assignment Percentage", validators=[DataRequired(), NumberRange(min=0,max=100)]
    )
    
    level1 = DecimalField('Level 1 Percentage', validators=[DataRequired(), NumberRange(min=0,max=100)])
    level2 = DecimalField('Level 2 Percentage', validators=[DataRequired(), NumberRange(min=0,max=100)])
    level3 = DecimalField('Level 3 Percentage', validators=[DataRequired(), NumberRange(min=0,max=100)])                        
    
    target = DecimalField('Target %', validators=[DataRequired(), NumberRange(min=0,max=100)])    
    scheme_year = IntegerField('Scheme Year', validators=[DataRequired(), NumberRange(min=2000,max=2099)])    
...
class quiz_mapping(FlaskForm):
    ...
    q1m = IntegerField(
        "Q-1", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q2m = IntegerField(
        "Q-2", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q3m = IntegerField(
        "Q-3", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q4m = IntegerField(
        "Q-4", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q5m = IntegerField(
        "Q-5", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q6m = IntegerField(
        "Q-6", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q7m = IntegerField(
        "Q-7", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q8m = IntegerField(
        "Q-8", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q9m = IntegerField(
        "Q-9", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q10m = IntegerField(
        "Q-10", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q11m = IntegerField(
        "Q-11", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q12m = IntegerField(
        "Q-12", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q13m = IntegerField(
        "Q-13", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q14m = IntegerField(
        "Q-14", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
    q15m = IntegerField(
        "Q-15", validators=[NumberRange(min=0), Optional(strip_whitespace=True)]
    )
...
class uploadsem(FlaskForm):
    scheme_year = IntegerField(
        "Scheme Year", validators=[DataRequired()], render_kw={"placeholder": "Enter Scheme Year"}
    )
    file = FileField(
        "Upload Student Marks", validators=[FileRequired()]
    )  # , FileAllowed('csv', 'Please Upload only csv files!')
...
class uploadsemwithPerc(FlaskForm):
    file = FileField(
        "Upload Student Marks", validators=[FileRequired()]
    )  # , FileAllowed('csv', 'Please Upload only csv files!')
    submit = SubmitField("Submit File")
...
class level_calculation(FlaskForm):
    level1 = DecimalField('Level 1 %', validators=[DataRequired(), NumberRange(min=0,max=100)])
    level2 = DecimalField('Level 2 %', validators=[DataRequired(), NumberRange(min=0,max=100)])
    level3 = DecimalField('Level 3 %', validators=[DataRequired(), NumberRange(min=0,max=100)])
    submit=SubmitField("Calculate Level")  
