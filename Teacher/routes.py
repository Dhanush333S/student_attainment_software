...
def add_course():
    ...
    if form.validate_on_submit():
        ...
        if d1+d2!=100 or s1+s2!=100 or t+q+assi!=100 :
            flash("Percentage sum is not Zero ! Recheck",category="danger")
            return render_template("admin.html", form=form)

        if level_3<level_2 or level_2<level_1 or level_3<level_1 or level_2<level_1:
            flash("Please Check ! Level 3 > Level 2 > Level 1",category="danger")
            return render_template("admin.html", form=form)

        scheme_year=form.scheme_year.data
        grade_marks_access = grade_marks.query.filter_by(scheme=scheme_year).first()
        
        if not grade_marks_access :
            flash(f"Grade entry for the {scheme_year} year is not found ! Set the Grade", category="danger")
            return render_template("admin.html", form=form)
...
def quizmappingco():
    ...
    if form.validate_on_submit():
        ...
        for i in range(0, 10):
            ...
            if marks_per_co[i]>0 and (i+1)>num_co:
...
def testmappingco():
    ...
    course_access = course.query.filter_by(coursecode=the_course_code).first()
    num_co = course_access.numberco
...
def assign_mapping():
    ...
    if form.validate_on_submit():
        ...
        if count == 1:
            ...
            flash(
                f"The Assignment Data has been Saved ", category="success"
            )
            return redirect(url_for("numberphases"))
...
@app.route("/downloadstudentupload", methods=['GET', 'POST'])
def download_student_upload_sample():
    sample_filename = "student_upload_sample.csv"
...
@app.route("/sem_perctomarks",methods=["GET","POST"])
def sem_perctomarks():
    form=uploadsemwithPerc()
    course_code = session["coursecode"]
    if course_code == "":
        flash("Please enter an appropriate Course Code!", category="danger")
        return render_template("newmain.html")
    access = course.query.filter_by(coursecode=course_code).first()
    num_co=access.numberco
    if request.method == "GET":
        return render_template("see_perc_upload.html",form=form)
    ...
    if form.validate_on_submit():
        ...
        file_path = app.config["UPLOAD_FOLDER"] + filename
        form.file.data.save(file_path)
        head = ["USN", "SCOp1","SCOp2","SCOp3","SCOp4","SCOp5","SCOp6","SCOp7","SCOp8","SCOp9","SCOp10"]
    with open(file_path, "r", newline="") as f:
            r = csv.DictReader(f)
            for i in r:
                if(i["USN"]==""):
                    continue
                check1 = see_perc.query.filter_by(
                    usn=i[head[0]], coursecode=course_code
                    ).first()
                check2 = student_co.query.filter_by(
                            usn=i["USN"], coursecode=course_code
                        ).first()
                if not check2:
                    flash(f'Internal not found for USN :{i["USN"]}',category="danger")
                    return render_template("newmain.html")
                if check1:
                    check1.co1p = ans1(i['SCOp1'])
                    check1.co2p = ans1(i['SCOp2'])
                    check1.co3p = ans1(i['SCOp3'])
                    check1.co4p = ans1(i['SCOp4'])
                    check1.co5p = ans1(i['SCOp5'])
                    check1.co6p = ans1(i['SCOp6'])
                    check1.co7p = ans1(i['SCOp7'])
                    check1.co8p = ans1(i['SCOp8'])
                    check1.co9p = ans1(i['SCOp9'])
                    check1.co10p = ans1(i['SCOp10'])
                    avg=((ans1(i['SCOp1'])+ans1(i['SCOp2'])+ans1(i['SCOp3'])+ans1(i['SCOp4'])+ans1(i['SCOp5'])+ans1(i['SCOp6'])+ans1(i['SCOp7'])+ans1(i['SCOp8'])+ans1(i['SCOp9'])+ans1(i['SCOp10']))/num_co)
                    if access.labyorn == "Y":
                        check2.sem_end_marks=round(avg*1.5, 2)
                    else :
                        check2.sem_end_marks=round(avg, 2)
                    db.session.commit()
                else :
                    to_add=see_perc(
                        usn=i[head[0]],
    ...
    if form.validate_on_submit():
        ...
        if updated_scheme_year!=scheme_year:
            ...
            if not grade_marks_access :
                ...
                return render_template("sem_grade_upload.html", form=form)
            scheme_year=updated_scheme_year
            access.scheme_year = scheme_year
            db.session.commit()
        ...
        with open(file_path, "r", newline="") as f:
            ...
            for i in r:
                ...
                grade = grade_marks.query.filter_by(scheme=scheme_year, alloted_grade=the_grade).first()
                if grade:
                    gp=grade.gpoint
                else:
                    gp=0

                # if the_grade in ["F", "NSSR", "NSAR", "X", "NE", "I", "W","","AB"]:
                #     gp = 0
                # else:
                ...
                if not other_one:
                    flash(f"Internal Marks for USN {i['USN']} Not Found!!", category="danger")
                    return render_template("newmain.html")
...
@app.route("/downloadseeprec",methods=["GET","POST"])
def download_see_perc():
    sample_filename="see_perc-format.csv"
    samples_path_mapping=app.config["SAMPLES_FOLDER"]+sample_filename
    with open(samples_path_mapping, "w", newline="") as f:
        head = ["USN", "SCOp1","SCOp2","SCOp3","SCOp4","SCOp5","SCOp6","SCOp7","SCOp8","SCOp9","SCOp10"]
        w = csv.DictWriter(f, fieldnames=head)
...
def dico_calc():
    ...
    for i in students:
        ...
        if i != "":
            ...
            den = 0
            if (not internal_row) or (not assign_row) or (not semester_access) or (not crsend_access):
                flash(f"Data Missing for USN : {i}", category="danger")
                return render_template("newmain.html")
            # check the 11th column of table
            sem_p=None
            if lab_status == "N":
                den = 100
                if semester_access.sem_end_marks!=None:
            ...
            if check1:
                s_co1p = new_co1p * intco + check1.co1p * semco
                s_co2p = new_co2p * intco + check1.co2p * semco
                s_co3p = new_co3p * intco + check1.co3p * semco
                s_co4p = new_co4p * intco + check1.co4p * semco
                s_co5p = new_co5p * intco + check1.co5p * semco
                s_co6p = new_co6p * intco + check1.co6p * semco
                s_co7p = new_co7p * intco + check1.co7p * semco
                s_co8p = new_co8p * intco + check1.co8p * semco
                s_co9p = new_co9p * intco + check1.co9p * semco
                s_co10p = new_co10p * intco + check1.co10p* semco
            else:     
                if sem_p==None:
                    flash(f"SEE Data Missing for USN : {i}", category="danger")
                    return render_template("newmain.html")
...
def level_calc():
    ...
    dicop = course_access.dicop
    scop = course_access.scop
    intco = (scop/(scop+1))
    semco = 1 - intco
    
    dco = (dicop/(dicop+1))
    ico = 1 - dco
    form=level_calculation()

    if request.method=='GET':
        form.level1.data=level_1
        form.level2.data=level_2
        form.level3.data=level_3
        return render_template("calculate_level.html",form=form)

    if form.validate_on_submit():
        updated_level_1=form.level1.data
        updated_level_2=form.level2.data
        updated_level_3=form.level3.data
        if updated_level_1!=level_1 or updated_level_2!=level_2 or updated_level_3!=level_3 :
            if updated_level_3<updated_level_2 or updated_level_2<updated_level_1 or updated_level_3<updated_level_1 or updated_level_2<updated_level_1:
                flash("Please Check ! Level 3 > Level 2 > Level 1",category="danger")
                return render_template("calculate_level.html", form=form)
            level_1=updated_level_1
            level_2=updated_level_2
            level_3=updated_level_3
            course_access.level1=updated_level_1
            course_access.level2=updated_level_2
            course_access.level3=updated_level_3
        ...
        co_columns = [f"co{i}p" for i in range(1, num_co + 1)]  # CO columns dynamically based on num_co

        # Calculate CIE level
        for i in studentco_access:
            sum_co = sum(getattr(i, col) for col in co_columns)
            avg_co = sum_co / num_co
            final_sum += avg_co
            final_count += 1
        ...
        final_d_level = intco * final_cie_level + semco * final_see_level
        ...
        indirect_ratio = sum(getattr(ia, f"co{i}p") for i in range(1, num_co + 1)) / num_co

        if indirect_ratio >= level_3:
            lev = 3
        elif indirect_ratio >= level_2:
            lev = 2
        elif indirect_ratio >= level_1:
            lev = 1
        else:
            lev = 0

        final = dco * final_d_level + ico * lev

        level_access = level.query.filter_by(coursecode=coursecode).update(
            dict(avg_indirect=indirect_ratio, indirect_level=lev, final_level=final)
        )
        db.session.commit()
    ...
    if form.errors != {}:
        for err_msg in form.errors.values():
            flash(f"There was an error : {err_msg}", category="danger")
    return render_template('calculte_level.html',form=form)
